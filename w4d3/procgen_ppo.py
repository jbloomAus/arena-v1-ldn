...
from utils import ppo_parse_args
...
def make_env(env_id: str, seed: int, idx: int, capture_video: bool, run_name: str, **kwargs):
    ...
    def thunk():
        ...
        reset_kwargs = dict(seed=seed) if 'seed' in inspect.getfullargspec(env.reset).args else dict()
...
def train_ppo(args: PPOArgs):
    ...
    if "procgen" in args.env_id:
        # env_name = args.env_id.split(":")[1].split("-")[1]
        # print("env_name", env_name)
        # envs = ProcgenGym3Env(num=1, env_name=env_name, start_level=0, num_levels=1)
...
if MAIN:
    ...
    args = PPOArgs(
        ...
        exp_name="starpilot better hyperparameters",
        track=True,
        capture_video=False,
        max_grad_norm=5, 
        vf_coef=0.19,
