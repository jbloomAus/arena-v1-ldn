...
def test_agent(Agent):
    ...
    agent = Agent(envs)
    assert sum(p.numel() for p in agent.critic.parameters()) == 4545, sum(p.numel() for p in agent.critic.parameters()) 
    assert sum(p.numel() for p in agent.actor.parameters()) == 4610, sum(p.numel() for p in agent.actor.parameters())
